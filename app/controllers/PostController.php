...
class PostController extends BaseController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		$records = $db->get('post', $limit, $fields);
		$data = new stdClass;
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			if(!empty($file_path)){
				$db = $this->GetModel();
				if($ext == 'csv'){
					$options = array('table' => 'post', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
					$data = $db->loadCsvData( $file_path , $options , false );
				}
				else{
					$data = $db->loadJsonData( $file_path, 'post' , false );
				}
				if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$rec_id = $db->insert('post',$modeldata);
			if(!empty($rec_id)){
				render_json($rec_id);
			}
			else{
