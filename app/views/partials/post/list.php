                ...
                <div class="container-fluid">
                    <div class="row ">
                        ...
                        <div  class="col-sm-3 comp-grid" :class="setGridSize">
                            <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/post/add'">
                            ...
                            <div  class=" animated fadeIn">
                                <div class="">
                                    <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                        <ul class="breadcrumb m-0 p-2">
                                            <li v-if="fieldname" class="breadcrumb-item">
                                                <router-link class="text-capitalize" to="/post">Post</router-link>
                                            </li>
                                        ...
                                        <table class="table" :class="tablestyle">
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    ...
                                                    <td><router-link :to="'/post/view/' +  data.id">{{data.id}}</router-link></td>
                                                    <td> {{ data.headline }} </td>
                                                    <td> {{ data.title }} </td>
                                                    <td> {{ data.category }} </td>
                                                    <td> {{ data.content }} </td>
                                                    ...
                                                    <th class="td-btn">
                                                        <div >
                                                            <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/post/view/' + data.id">
                                                            <i class="fa fa-eye"></i> 
                                                            </router-link>
                                                            <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/post/edit/' + data.id">
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="" post-action="post/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
	var PostListComponent = Vue.component('postList', {
		template: '#postList',
